...
def main():
    ...
    dataloader = DataLoader(
        dataset, batch_size=BATCH_SIZE, num_workers=6, pin_memory=True, drop_last=True
    )
    ...
    for batch_num, (inputs, targets) in tqdm(
        enumerate(dataloader), unit="Batch", total=len(dataset) // BATCH_SIZE
    ):
        ...
        if (batch_num % 100) == 0:
            writer.add_scalar("train/loss", running_loss, batch_num)
            running_loss = 0.0
            writer.add_figure(
